...
tlib_file = function(options) {
	...
	if (typeof options !== undefined && options.filename && options.virtualRoot && options.success && options.error){
		tlib_logger.info("All parameters of options Object are defined");
	} else {
		tlib_logger.err("Define all parameters of options Object !");
	}		
	...
	try {
		tizen.filesystem.resolve(options.virtualRoot, function(dir) {
			gDocumentsDir = dir;
			tlib_logger.info(options.virtualRoot + " resolved");
			if (resolve()) options.success();
		}, function(e) {
			tlib_logger.err("tizen.filesystem.resolve() error: " + e.message);
			options.error();
		}, "rw");
	} catch (exc) {
		tlib_logger.err("tizen.filesystem.resolve() exception: " + exc.message);
		options.error();
	}
	...
	var create = function() {
		try {
			...
			tlib_logger.info("File created");
			return true;
		} catch (exc) {
			if (exc.name === "IOError") {
				tlib_logger.warn("Create file failed, because file already exists.");
				return true;
			} else {
				tlib_logger.err("Create file exception: " + exc.message);
				return false;
			}
		}
	}
	...
	var resolve = function() {
		tlib_logger.info("this.resolve function");
		try {
			file = gDocumentsDir.resolve(filename);
			tlib_logger.info("File resolved");
			return true; 
		} catch (exc) {
			tlib_logger.info(exc.name);
			if (exc.name === "NotFoundError" || "IOError") {
				tlib_logger.warn('File not found, so it will be created (exc message: ' + exc.message + ')');
				if (create()) 
					if (resolve())
						return true;
			} else {
				tlib_logger.err('Resolve file exception: ' + exc.message);
				return false;
			}
		}
	}
			...
			try {
				file.openStream('w', function(fileStream) {
					try {
						...
						tlib_logger.err(exc.message);
						callback.error();
					}
				}, function(e) {
					tlib_logger.err("openStream error: " + e.message);
					callback.error();
				});
			} catch (exc) {
				tlib_logger.err('Write function exception: ' + exc.message);
				callback.error();
			}
	}
	...
	this.append = function(text, callback) {
		tlib_logger.info("Appending to file");
			try {
				file.openStream('a', function(fileStream) {
					...
					tlib_logger.info("openStream error: " + e.message);
					callback.error();
				});
			} catch (exc) {
				tlib_logger.err('Append function exception: ' + exc.message);
				callback.error();
			}
	}
			...
			try {
				...
				file.openStream('r', function(fileStream) {
					try {
						...
						tlib_logger.err('Read function exception: ' + exc.message);
						callback.error();
					}
				}, function(e) {
					tlib_logger.err("openStream error: " + e.message);
				...
				tlib_logger.err('file.openStream() exception: ' + exc.message);
				callback.error();
			}
	}
